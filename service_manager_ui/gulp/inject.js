...
gulp.task('inject', ['scripts', 'styles', 'injectAuth','injectLogin', 'inject404','resetPWD', 'copyVendorImages'], function () {
  var injectStyles = gulp.src([
    path.join(conf.paths.tmp, '/serve/app/main.css'),
    path.join(conf.paths.tmp, '/serve/app/css/bootstrap-datetimepicker.min.css'),
...
gulp.task('injectLogin', ['scripts', 'styles', 'injectAuth', 'inject404','resetPWD', 'copyVendorImages'], function () {
  var injectStyles = gulp.src([
  ...
  var injectScripts = gulp.src([
    path.join(conf.paths.src, '/app/login/*.module.js'),
    path.join(conf.paths.src, '/app/login/*.js'),
...
gulp.task('resetPWD', function () {
  return injectAlone({
    css: [path.join('!' + conf.paths.tmp, '/serve/app/vendor.css'), path.join(conf.paths.tmp, '/serve/app/404.css')],
    paths: path.join(conf.paths.src, '/resetPassword.html')
  })
});
